...
## Purpose
Download Florida Division of Corporations bulk data, cross-reference entity-owned properties from Module 1 to resolve LLC/Corp/Trust names to the actual human owners (registered agents, officers, directors).

## Data Source

**SunBiz Bulk Data (FTP):**
- Portal: https://dos.fl.gov/sunbiz/other-services/data-downloads/
- Quarterly dumps: https://dos.fl.gov/sunbiz/other-services/data-downloads/quarterly-data/
- Daily incremental: https://dos.fl.gov/sunbiz/other-services/data-downloads/daily-data/
- Public FTP credentials listed on the data downloads page
- Format: Fixed-width or delimited text files with file definitions on the download page
- Coverage: All FL corporations, LLCs, limited partnerships

**SunBiz Individual Search (Fallback):**
- URL: https://search.sunbiz.org/Inquiry/CorporationSearch/ByName
- Use for targeted lookups when bulk data doesn't resolve

## What SunBiz Data Contains

| Field | Description | Use |
|---|---|---|
| Entity Name | Legal name of LLC/Corp/LP | Match to FDOR owner name |
| Entity Number | FL filing number | Unique identifier |
| Filing Type | LLC, Corp, LP, etc. | Entity classification |
| Status | Active, Inactive, Dissolved | Filter for active only |
| Filing Date | Date of formation | Recency indicator |
| Registered Agent Name | Person or entity designated as agent | **PRIMARY: this is often the owner** |
| Registered Agent Address | Physical address of agent | Contact address |
| Officer/Director Names | Up to 6 officers listed in annual reports | **SECONDARY: human names behind entity** |
| Officer Titles | Manager, Member, President, etc. | Role identification |
| Officer Addresses | Listed addresses for officers | Contact addresses |
| Principal Address | Main business address | Business location |
| Mailing Address | Entity mailing address | Contact address |

## Script: `scripts/02_sunbiz_resolve.py`
...
- SunBiz does not publish official API rate limits
- Default: 2-second delay between requests
- Max 500 lookups per run (configurable) — ~17 minutes per batch
- Cache all results to avoid repeat lookups
- Set proper User-Agent header

## Expected Resolution Rate

- **70-80%** of entities should resolve to a human name via registered agent or officer
- **20-30%** will have another entity as registered agent (e.g., "CT Corporation System") — these are harder to resolve and may need manual review

## Scaling

For full-scale operation across all 67 counties:
1. Run SunBiz bulk FTP download (quarterly files) to build local lookup table
2. Match entity names directly against bulk data — no web scraping needed
3. Only fall back to web scraping for entities not found in bulk data

This eliminates rate limiting as a constraint for scale.
