# Module 1: FDOR Property Data Ingestion

## Purpose
Download Florida Department of Revenue NAL (Name-Address-Legal) and SDF (Sales Data File) files, filter for non-owner-occupied residential investment properties, and produce a clean investor property dataset.

## Data Source

**NAL Files (Property Records):**
- URL: `https://floridarevenue.com/property/dataportal/Documents/PTO%20Data%20Portal/Tax%20Roll%20Data%20Files/NAL/2025F/`
- Format: ZIP files containing CSV, one per county
- Naming pattern: `{County Name} {County Code} Final NAL 2025.zip`
- Documentation: https://floridarevenue.com/property/dataportal/Documents/PTO%20Data%20Portal/User%20Guides/2025%20Users%20guide%20and%20quick%20reference/2025_NAL_SDF_NAP_Users_Guide.pdf

**SDF Files (Sales Data):**
- URL: Same portal structure under SDF subfolder
- Format: ZIP files containing CSV, one per county

## Key NAL Fields (from User's Guide)

| Field Position | Field Name | Description | Use |
|---|---|---|---|
| 1 | CO_NO | County code (01-67) | County identifier |
| 2 | PARCEL_ID | Parcel identification number | Unique property ID |
| 74-79 | OWN_NAME, OWN_ADDR1, OWN_ADDR2, OWN_CITY, OWN_STATE, OWN_ZIP | Owner mailing info | Contact info |
| 91-96 | SITE_ADDR, SITE_CITY, SITE_ZIP | Property physical address | Location |
| 8 | DOR_UC | Department of Revenue Use Code | Property type classification |
| 11 | JV | Just (market) value | Property value |
| 33 | AV_HMSTD | Assessed value homestead | Homestead indicator (0 = not homesteaded) |
| 110-158 | Exemption fields | Various exemption types | Confirm non-homestead |

## DOR Use Codes for Residential Investment Properties
...
04 = Condominium
05 = Cooperatives
06 = Retirement Homes (non-profit)
07 = Miscellaneous Residential
08 = Multi-Family (10+ units)
09 = Residential Common Elements
```

**Target codes for DSCR prospects: 01, 02, 03, 04, 08**

## Script: `scripts/01_fdor_download_filter.py`
...
For Palm Beach County alone:
- Total parcels: ~600,000+
- Residential: ~400,000
- Non-homesteaded residential: ~100,000-150,000
- Aggregated to unique investor owners: ~50,000-80,000

## Notes

1. **Column names may vary** between counties and years. The script handles this with fuzzy column matching but may need adjustment after first test run.
2. **File naming on FDOR portal** may not match exactly. First run should manually verify the download URLs and adjust patterns.
3. **Memory:** Large counties (Miami-Dade, Broward) may produce CSV files >500MB. Process one county at a time if memory is constrained.
4. **Run time:** Download + filter for one county takes ~2-5 minutes depending on size and connection speed.
